...
_TRANSLATIONS = {
    LANG_ZH_CN: {
        ...
        "Show lyric alongside automation cues": "在自动化提示旁显示歌词",
        "Optional script-wide notes": "可选的脚本整体备注",
        "1. Pick a cue or lyric time here. This top list is your timing map.": "1. 在这里选择提示点或歌词时间。上方列表就是你的时间地图。",
        "2. Edit the selected cue here. Add comments only if they help the operator.": "2. 在这里编辑当前选中的提示点。只有在对操作员有帮助时才添加备注。",
        "3. Pick a Companion command on the right, then add it into the selected cue.": "3. 在右侧选择一个 Companion 命令，然后把它加入当前选中的提示点。",
        "Add Cue At Current Timestamp": "在当前时间戳添加提示点",
        "Add Cue On Selected Lyric Time": "在所选歌词时间添加提示点",
        "Add Selected Command To Cue": "将所选命令加入提示点",
        "Remove Selected Command": "移除所选命令",
        "Select a cue row to edit it. Cues without commands are not saved.": "请选择一个提示点行进行编辑。没有命令的提示点不会被保存。",
        "This cue is edited inline. Use the right panel to add Companion commands.": "这个提示点可直接在当前窗口编辑。请使用右侧面板添加 Companion 命令。",
        "Press / Release": "按下 / 释放",
        "Normal": "普通",
