...
class VocalRemovedBatchDialog(QDialog):
    def __init__(
        ...
        *,
        ...
        self.table = QTableWidget(self._row_count, 5, self)
        self.table.setHorizontalHeaderLabels(
            [tr(action_header), tr("Title"), tr("Filename"), tr(target_header), tr("Page")]
        )
        ...
        self.table.horizontalHeader().setSectionResizeMode(1, self.table.horizontalHeader().Interactive)
        self.table.horizontalHeader().setSectionResizeMode(2, self.table.horizontalHeader().Interactive)
        self.table.horizontalHeader().setSectionResizeMode(3, self.table.horizontalHeader().Interactive)
        ...
        for row_index, (song_title, audio_path, target_path, location, enabled) in enumerate(rows):
            ...
            title_item.setToolTip(song_title)
            ...
            self.table.setItem(row_index, 2, audio_item)

            target_text = os.path.basename(target_path) if target_path else tr("(not found)")
            ...
            self.table.setItem(row_index, 4, location_item)

        root.addWidget(self.table, 1)

        action_row = QHBoxLayout()
        select_all_btn = QPushButton(tr("Select All Eligible"), self)
        clear_all_btn = QPushButton(tr("Unselect All"), self)
