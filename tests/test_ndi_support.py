...
def test_probe_ndi_capability_reports_missing_binding(monkeypatch):
    ...
    monkeypatch.setattr(ndi_support, "_bundled_runtime_candidates", lambda: [])
    ...
    assert "cyndilib is not installed" in status.availability_reason.lower()
...
def test_probe_ndi_capability_reports_missing_binding_even_when_sdk_exists(monkeypatch):
    ...
    assert status.ready is False
    assert "cyndilib is not installed" in status.availability_reason.lower()
    assert "runtime/sdk was detected" in status.availability_reason.lower()


def test_probe_ndi_capability_reports_runtime_missing(monkeypatch):
    monkeypatch.setattr(ndi_support.importlib.metadata, "version", lambda _name: "0.1.1")
    ...
    assert status.ready is False
    assert "runtime binaries were not detected" in status.availability_reason.lower()
...
def test_probe_ndi_capability_reports_ready(monkeypatch):
    ...
    assert status.ndi_python_version == "0.1.1"
    assert status.runtime_paths == ["/opt/ndi"]
    assert status.sdk_paths == ["/opt/ndi-sdk"]


def test_probe_ndi_capability_reports_ready_with_bundled_runtime(monkeypatch):
    monkeypatch.setattr(ndi_support.importlib.metadata, "version", lambda _name: "0.1.1")
    monkeypatch.setattr(ndi_support.importlib, "import_module", lambda _name: object())
    monkeypatch.setattr(ndi_support, "_runtime_candidates", lambda: ("NDI_RUNTIME_DIR_V6", []))
    monkeypatch.setattr(ndi_support, "_sdk_candidates", lambda: [])
    monkeypatch.setattr(ndi_support, "_bundled_runtime_candidates", lambda: ["/venv/site-packages/cyndilib/wrapper/bin"])
    monkeypatch.setattr(ndi_support, "_existing_paths", lambda values: list(values))
    monkeypatch.setattr(ndi_support, "_env_dir", lambda _name: "")
    ...
    assert status.ready is True
    assert status.bundled_runtime_paths == ["/venv/site-packages/cyndilib/wrapper/bin"]
    assert "bundled runtime" in status.availability_reason.lower()
